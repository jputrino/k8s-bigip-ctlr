...
.. important::
   ...
   Create all |kctlr| objects in the ``kube-system`` `namespace`_, unless otherwise specified in the deployment instructions.
...
#. Create `Kubernetes Secrets`_ for the following:

   - `Docker login credentials`_ - required if you need to pull the container image from a private Docker registry.

   - `SSL profiles`_ - required if you want to create a new SSL profile on the BIG-IP device.

.. seealso::

   **If you're using OpenShift**, see

   - :ref:`Add your BIG-IP device to an OpenShift Cluster <bigip-openshift-setup>`.
   - :ref:`Set up the BIG-IP Controller to use RBAC <#tbd>`
...
.. _Docker login credentials: https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/